...
function fakeIngestBuffer() {
  return {
    ...
    enqueueCommitted: vi.fn().mockResolvedValue({ runId: 'com-1', traceId: 'tr-2' }),
  }
}
...
function fakeHandlers(engine: ReturnType<typeof fakeEngine>) {
  return {
    start: vi.fn().mockImplementation(async (input: any) => engine.start(input)),
    getStatus: vi.fn().mockImplementation(async (input: any) => engine.getStatus(input.runId, input.tenantId)),
    startBatch: vi.fn().mockImplementation(async (input: any) => engine.startBatch(input.workflows)),
    startBatchCopy: vi.fn().mockImplementation(async (input: any) => engine.startBatchCopy(input.workflows)),
    cancel: vi.fn().mockImplementation(async (input: any) => engine.cancel(input.runId, input.tenantId)),
    signal: vi.fn().mockImplementation(async (input: any) => engine.signal(input.runId, input.tenantId, input.signalName, input.data)),
    query: vi.fn().mockImplementation(async (input: any) => engine.query(input.runId, input.tenantId, input.queryName)),
    submitHumanInput: vi.fn().mockImplementation(async (input: any) => engine.submitHumanInput(input)),
    listWorkflows: vi.fn().mockResolvedValue([
      { name: 'order-flow', version: '1', stepCount: 2 },
    ]),
...
describe('agentsRouter', () => {
  ...
  describe('POST /start-async', () => {
    it('uses ingestBuffer.enqueue for buffered workflows', async () => {
      ...
      expect(res.body).toEqual({ runId: 'buf-1', traceId: 'tr-1', status: 'QUEUED' })
    })
    ...
    it('uses ingestBuffer.enqueueCommitted for committed workflows', async () => {
      ...
      expect(res.body).toEqual({ runId: 'com-1', traceId: 'tr-2', status: 'COMMITTED' })
    })
  ...
  describe('error mapping', () => {
    it('maps WORKFLOW_RUN_NOT_FOUND to 404', async () => {
      handlers.getStatus.mockRejectedValueOnce(
        Object.assign(new Error('Run not found'), { code: 'WORKFLOW_RUN_NOT_FOUND' }),
      )
      const res = await request(app, 'POST', '/api/status', { runId: 'missing' })
      expect(res.status).toBe(404)
  ...
  describe('GET /', () => {
    it('calls listWorkflows with tenantId', async () => {
      ...
      expect(res.status).toBe(200)
      expect(handlers.listWorkflows).toHaveBeenCalledWith({ tenantId: 'tenant-1' })
    })
  })
  ...
  describe('POST /signal', () => {
    it('calls signal handler with body + tenantId', async () => {
      const body = { runId: 'run-1', signalName: 'approve', data: { ok: true } }

...
export const STEP_SCHEMAS: Record<string, (output: any) => boolean> = {
  ingest_feedback: (o) =>
    typeof o.rawText === 'string' && typeof o.source === 'string' && typeof o.timestamp === 'number',
  structure_feedback: (o) =>
    typeof o.title === 'string' && Array.isArray(o.requirements) && typeof o.priority === 'string',
  generate_plan: (o) =>
    Array.isArray(o.tasks) && o.tasks.every((t: any) => t.title && t.description && t.estimate),
  create_tasks: (o) =>
    Array.isArray(o.issueIds) && o.issueIds.length > 0,
  implement_code: (o) =>
    typeof o.prId === 'string' && typeof o.branch === 'string' && Array.isArray(o.files),
  review_code: (o) =>
    typeof o.approved === 'boolean' && typeof o.comment === 'string',
  generate_ui: (o) =>
    Array.isArray(o.artifacts) && o.artifacts.length > 0,
  validate_output: (o) =>
    typeof o.valid === 'boolean' && Array.isArray(o.checks),
  complete_workflow: (o) =>
...
export function createSDLCExecutor(ctx: SDLCContext): FunctionStepExecutor {
  ...
  executor.register('ingest_feedback', async (payload: StepPayload): Promise<StepResult> => {
    if (shouldFail('ingest_feedback')) throw new Error('Agent timeout: ingest_feedback')
  ...
  executor.register('structure_feedback', async (payload: StepPayload): Promise<StepResult> => {
    if (shouldFail('structure_feedback')) throw new Error('Agent timeout: structure_feedback')
    ...
    return {
      output: {
        ...
        requirements: [
          { id: 'REQ-1', description: 'Primary requirement from feedback', priority: 'high' },
  ...
  executor.register('generate_plan', async (payload: StepPayload): Promise<StepResult> => {
    if (shouldFail('generate_plan')) throw new Error('Agent timeout: generate_plan')
    ...
    return {
      output: {
        tasks: [
          { title: 'Create data model', description: 'Define schema', estimate: '2h', type: 'backend' },
          { title: 'Build API endpoint', description: 'REST handler', estimate: '3h', type: 'backend' },
          { title: 'Create UI component', description: 'React form', estimate: '4h', type: 'frontend' },
        ],
        valid: true,
      },
    }
  ...
  executor.register('implement_code', async (payload: StepPayload, context?: StepExecutionContext): Promise<StepResult> => {
    ...
    const files = [
      { path: 'src/model.ts', content: '// Generated model\nexport interface Model {}' },
  ...
  executor.register('validate_output', async (payload: StepPayload): Promise<StepResult> => {
    if (shouldFail('validate_output')) throw new Error('Agent timeout: validate_output')

    ctx.tracker.record('agent', 'validate_output', payload.input, payload.stepName)
  ...
  executor.register('complete_workflow', async (payload: StepPayload): Promise<StepResult> => {
    ctx.tracker.record('agent', 'complete_workflow', payload.input, payload.stepName)

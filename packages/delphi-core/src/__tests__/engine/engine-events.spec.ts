...
describe('WorkflowEngine.onEngineEvent', () => {
  ...
  beforeAll(async () => {
    ...
    executor.register('hitl', async (): Promise<StepResult> => ({
    ...
    const wfBoom = WorkflowBuilder.create('wf_boom')
      .version('1.0.0').defaultRetries(0)
      .step('explode', { executorType: 'function', executorConfig: { handler: 'boom' } })
      .build()

    const wfHitl = WorkflowBuilder.create('wf_hitl')
      .version('1.0.0').defaultRetries(0)
      .step('approve', { executorType: 'function', executorConfig: { handler: 'hitl' } })
      .build()
    ...
    workerHandle = await connector.listen({
      tasks: [
        { taskName: 'workflow_step_light',     handle: (d: unknown) => stepTask.handle(d as StepPayload), concurrency: 5 },
        { taskName: 'workflow_step_heavy',     handle: (d: unknown) => stepTask.handle(d as StepPayload), concurrency: 2 },
        { taskName: 'workflow_step_ai',        handle: (d: unknown) => stepTask.handle(d as StepPayload), concurrency: 2 },
        { taskName: 'workflow_step_sandbox',   handle: (d: unknown) => stepTask.handle(d as StepPayload), concurrency: 2 },
      ],
    })
  }, 60_000)
  ...
  it('happy path: emits run.started → step.running → step.completed → run.completed', async () => {
    ...
    const started = events.find(e => e.runId === runId && e.type === 'run.started')!
    ...
    const completed = events.find(e => e.runId === runId && e.type === 'step.completed')!
    ...
    const runCompleted = events.find(e => e.runId === runId && e.type === 'run.completed')!
  ...
  it('CRITICAL: PG row reflects new state when event fires (no race)', async () => {
    ...
    const runRow = await db.selectFrom('workflow_runs')
      .select('status')
      .where('id', '=', runId)
      .executeTakeFirst()
  ...
  it('emits step.failed (terminal=true) and run.completed (status=FAILED) on failure', async () => {
    ...
    await waitFor(() => events.some(e => e.runId === runId && e.type === 'run.completed'))

    const failed = events.find(e => e.runId === runId && e.type === 'step.failed')!
  ...
  it('emits step.human_requested when waitForHuman is set', async () => {
    ...
    await waitFor(() => events.some(e => e.runId === runId && e.type === 'step.human_requested'))

    const requested = events.find(e => e.runId === runId && e.type === 'step.human_requested')!
    ...
    expect(events.find(e => e.runId === runId && e.type === 'run.completed')).toBeUndefined()
  }, 15_000)

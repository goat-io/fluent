...
describe('Trace Propagation', () => {
  ...
  function createEngine(extraConfig?: { eventIngestion?: EventIngestionService }) {
    ...
    const wf = WorkflowBuilder.create('trace-wf')
      ...
      .step('step2', { dependsOn: ['step1'], executorType: 'function', executorConfig: { handler: 'echo' } })
      .build()
  ...
  it('traceId propagates to external actions', async () => {
    ...
    executor.register('callExternal', async (p: StepPayload, ctx?: StepExecutionContext): Promise<StepResult> => {
      await ctx!.externalActions.execute(
        ...
        async (req) => ({ externalId: 'pr-42', data: { url: 'https://github.com/test/pr/42' } }),
      )
    ...
    const wf = WorkflowBuilder.create('trace-external')
      .step('call', { executorType: 'function', executorConfig: { handler: 'callExternal' } })
      .build()
    ...
    const { runId } = await engine.start({
      workflowName: 'trace-external', tenantId: 'test-tenant', input: {},
      traceId,
    })
    ...
    await db.updateTable('workflow_steps')
      ...
      .where('status', '=', 'QUEUED')
      .execute()
    const ctx: StepExecutionContext = { externalActions: engine.externalActions }
    const result = await executor.execute(payload, ctx)
    await engine.onStepCompleted(payload.workflowRunId, payload.stepName, payload.tenantId, result)

    // Verify the external action has the traceId
    const actions = await db.selectFrom('external_actions')
      .selectAll()
  ...
  it('event-ingested traceId is persisted in workflow_events', async () => {
    const eventIngestion = new EventIngestionService({ db, skipAutoProcess: true })
    ...
    const event = await db.selectFrom('workflow_events')
      .selectAll().where('id', '=', eventId).executeTakeFirstOrThrow()
  ...
  it('startBatch assigns unique traceIds to each workflow', async () => {
    ...
    const run1 = await db.selectFrom('workflow_runs')
      .selectAll().where('id', '=', results[0].runId).executeTakeFirstOrThrow()
    const run2 = await db.selectFrom('workflow_runs')
      .selectAll().where('id', '=', results[1].runId).executeTakeFirstOrThrow()

...
describe('Temporal-Inspired Features', () => {
  ...
  beforeEach(async () => {
    ...
    executor.register('longRunning', async (p: StepPayload): Promise<StepResult> => ({
      output: { done: true },
    }))
  })
  ...
  async function executeStep(engine: WorkflowEngine, job: any) {
    ...
    const stepEntity = await db.selectFrom('workflow_steps').selectAll()
    ...
    if (stepEntity && stepEntity.status === 'QUEUED') {
      await db.updateTable('workflow_steps').set({
        ...
        startedAt: new Date(),
        updatedAt: new Date(),
      }).where('id', '=', stepEntity.id).execute()
    }
  ...
  describe('Heartbeat', () => {
    it('records heartbeat on a running step', async () => {
      const wf = WorkflowBuilder.create('hb_test')
        .step('work', { executorType: 'function', executorConfig: { handler: 'longRunning' } })
        .build()
      ...
      // Send heartbeat
      await engine.heartbeat(runId, 'work', 'test-tenant', { processed: 500, total: 10000 })

      // Verify
      const updated = await db.selectFrom('workflow_steps').selectAll()
        .where('id', '=', step!.id)
        .executeTakeFirst()
      expect(updated!.lastHeartbeatAt).toBeDefined()
      const heartbeatData = typeof updated!.lastHeartbeatData === 'string'
    ...
    it('creates heartbeat log entry', async () => {
      ...
      const logData = typeof logs[0].data === 'string'
  ...
  describe('Signals', () => {
    it('persists signal in database', async () => {
      ...
      await engine.signal(runId, 'test-tenant', 'custom_event', { key: 'value' })

      const signals = await db.selectFrom('workflow_signals').selectAll()
        .where('workflowRunId', '=', runId)
        .execute()
      expect(signals).toHaveLength(1)
      expect(signals[0].signalName).toBe('custom_event')
      const signalData = typeof signals[0].data === 'string'
    ...
    it('executes signal handler when registered', async () => {
      ...
      const wf = WorkflowBuilder.create('sig_handler')
        .step('a', { executorType: 'function', executorConfig: { handler: 'echo' } })
        .onSignal('approve', async (ctx, data) => {
          handlerCalled = true
          receivedData = data
        })
        .build()

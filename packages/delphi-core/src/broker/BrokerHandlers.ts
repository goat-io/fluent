...
import type { AgentRegistry, AgentCapabilities, RegisteredAgent } from './AgentRegistry.js'
...
export function createBrokerHandlers(config: BrokerHandlersConfig) {
  ...
  return {
    ...
    async generateAgentToken(input: { tenantId: string }): Promise<{
      ...
      await db.insertInto('agent_tokens').values({
        id,
      ...
      logger?.info(`Agent registration token generated for tenant ${input.tenantId}`)
    ...
    async register(input: {
      ...
      // Mark token as used
      await db.updateTable('agent_tokens')
    ...
    async nextJob(input: {
      ...
      job: { id: string; type: 'step' | 'task'; queue: string; payload: Record<string, unknown> } | null
    }> {
      ...
      while (Date.now() < deadline) {
        ...
        if (job) {
          return {
            job: { id: job.id, type: job.type, queue: job.queue, payload: job.payload },
          }
        }
    ...
    async stepResult(input: {
      ...
      verifyAgent(registry, input.agentId, input.secret)
      const accepted = registry.completeJob(input.agentId, input.jobId, input.result)
      return { accepted }
    },
...
function verifyAgent(
  ...
  if (secretHash !== agent.secretHash) throw new Error('Invalid agent secret')

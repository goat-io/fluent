...
import type { Skill } from '@goatlab/agents-core'
...
function makeSearchSkill(): Skill {
  return {
    ...
    execute: vi.fn().mockResolvedValue({ results: [{ title: 'Result 1', url: 'https://example.com' }] }),
  }
}
...
describe('AIStepExecutor Tool-Calling Loop', () => {
  ...
  it('executes tool-calling loop: tool call → skill execute → final answer', async () => {
    ...
    mockChat.mockResolvedValueOnce(toolCallResponse).mockResolvedValueOnce(finalResponse)
  ...
  it('accumulates usage across multiple tool turns', async () => {
    ...
    mockChat
      .mockResolvedValueOnce({
        ...
        toolCalls: [{ id: 'tc-1', name: 'web_search', arguments: { query: 'a' } }],
      })
      .mockResolvedValueOnce({
        content: '',
        model: 'openai/gpt-4o',
        usage: { promptTokens: 200, completionTokens: 80, totalTokens: 280 },
        toolCalls: [{ id: 'tc-2', name: 'web_search', arguments: { query: 'b' } }],
      })
      .mockResolvedValueOnce({

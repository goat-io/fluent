    ...
    execute: vi.fn().mockImplementation(async (prompt, ctx) => ({
      ...
      payload:
        role === AgentRole.PROPOSER
          ? { content: 'Test proposal', rationale: 'Reasoning', confidence: 0.8 }
          : {
              proposalId: 'current',
              concerns: assessment === 'approve' ? [] : [{ severity: 'major', description: 'Issue found' }],
...
function defaultConfig(overrides: Partial<AgreementSessionConfig> = {}): AgreementSessionConfig {
  return {

...
describe('CircuitBreaker', () => {
  ...
  describe('error classification', () => {
    it('respects shouldTrip filter', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        try { await cb.execute(async () => { throw { status: 400, message: 'bad request' } }) } catch {}
      }
      expect(cb.getState()).toBe('CLOSED')

      // 500 errors SHOULD trip
      for (let i = 0; i < 2; i++) {
        try { await cb.execute(async () => { throw { status: 500, message: 'server error' } }) } catch {}
      }
      expect(cb.getState()).toBe('OPEN')
    })
  })
  ...
  describe('metrics', () => {
    it('tracks call counts', async () => {
      const cb = new CircuitBreaker({ failureThreshold: 10, resetTimeoutMs: 1000 })

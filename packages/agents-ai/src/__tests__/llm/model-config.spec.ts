...
import { MODEL_PRESETS, STRATEGY_MODEL_MAPPINGS, DEFAULT_ROLE_MODELS } from '../../llm/ModelConfig.js'
...
describe('ModelConfig', () => {
  describe('MODEL_PRESETS', () => {
    ...
    it('has Ollama models', () => {
      ...
      expect(MODEL_PRESETS['llama-3-local'].baseUrl).toBe('http://localhost:11434')
    })
  ...
  describe('STRATEGY_MODEL_MAPPINGS', () => {
    ...
    it('all strategies reference valid presets', () => {
      for (const [strategyName, mapping] of Object.entries(STRATEGY_MODEL_MAPPINGS)) {
        const models = [
          mapping.proposer,
          ...(Array.isArray(mapping.reviewer) ? mapping.reviewer : [mapping.reviewer]),
          ...(mapping.arbiter ? [mapping.arbiter] : []),
        ]
        for (const m of models) {
          if (typeof m === 'string') {
            expect(MODEL_PRESETS[m], `Strategy "${strategyName}" references unknown preset "${m}"`).toBeDefined()
          }
        }
      }
    })
...
describe('ModelSelector', () => {
  ...
  it('throws for unknown preset', () => {
    expect(() => modelSelector.resolveModelConfig('nonexistent')).toThrow(/Unknown model preset/)
  })
  ...
  it('resolves model for role', () => {
    const proposer = modelSelector.resolveModelForRole('code-review', 'proposer')
  ...
  it('recommends budget models', () => {
    ...
    expect(typeof models.proposer === 'string' || models.proposer.model).toBeDefined()
  })

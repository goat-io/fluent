...
export class RiskGuard {
  ...
  async checkRisks(context: {
    messages: Array<{ tokenUsage?: { total: number }; payload?: any; step?: string }>
    turnCount: number
    ...
    if (totalTokens > this.config.maxTokensPerTurn * Math.max(context.turnCount, 1)) {
      return { safe: false, reason: `Token budget exceeded: ${totalTokens} tokens used` }
    }
    ...
    if (context.turnCount >= 2) {
      ...
      if (proposals.length >= 2) {
        ...
        if (last === prev) {
          return { safe: false, reason: 'Cyclical arguments detected: identical proposals' }
        }
      }
    }

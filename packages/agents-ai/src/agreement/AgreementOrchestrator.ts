...
export class AgreementOrchestrator {
  ...
  private async handleRefine(): Promise<void> {
    await this.handlePropose(
      `Refine proposal based on critiques: ${JSON.stringify(
        this.messages.filter(m => m.step === AgreementState.CRITIQUE).map(m => m.payload),
      )}`,
    )
  }

  private collectVotes(
    critiques: AgreementMessage[],
  ): VotePayload[] {
    return critiques.map(c => {
      const payload = c.payload as CritiquePayload
      return {
        proposalId: payload.proposalId ?? c.id,
        vote: payload.overallAssessment === 'approve' ? 'approve' as const : 'reject' as const,
  ...
  private createResult(): ConsensusResult {
    const proposals = this.messages.filter(m => m.step === AgreementState.PROPOSE)
    const latestProposal = proposals[proposals.length - 1]
    const critiques = this.messages.filter(m => m.step === AgreementState.CRITIQUE)
    const votes = this.collectVotes(critiques)
    const score = this.calculateConsensus(votes)
  ...
  getState(): AgreementState { return this.currentState }
  getMessages(): AgreementMessage[] { return [...this.messages] }

...
export class AgreementStepExecutor implements StepExecutor {
  ...
  async execute(payload: StepPayload): Promise<StepResult> {
    const config = payload.executorConfig as {
      ...
      reviewers?: Array<{ model: string; focus?: string; systemPrompt?: string }>
    ...
    // Proposer
    const proposerModel = config.proposer?.model
      ?? modelSelector.resolveModelForRole(strategy, 'proposer').model
    agents.push(
      this.createAgent('proposer-1', AgentRole.PROPOSER, proposerModel, config.proposer?.systemPrompt),
    )
    ...
    if (config.reviewers?.length) {
      for (let i = 0; i < config.reviewers.length; i++) {
        ...
        agents.push(
          this.createAgent(`reviewer-${i + 1}`, AgentRole.REVIEWER, r.model, r.systemPrompt),
        )
      }
    } else {
      // Default reviewers from strategy
      const mapping = modelSelector.getStrategyModels(strategy)
      const reviewerModels = Array.isArray(mapping.reviewer) ? mapping.reviewer : [mapping.reviewer]
      for (let i = 0; i < reviewerModels.length; i++) {
        const resolved = modelSelector.resolveModelConfig(reviewerModels[i])
        agents.push(
          this.createAgent(`reviewer-${i + 1}`, AgentRole.REVIEWER, resolved.model),
        )
      }
    }
    ...
    if (config.arbiter) {
      agents.push(
        this.createAgent('arbiter-1', AgentRole.ARBITER, config.arbiter.model, config.arbiter.systemPrompt),
      )
    }
    ...
    const goal = typeof payload.input === 'string'
      ? payload.input
  ...
  private createAgent(
    ...
    return {
      ...
      execute: async (prompt, context) => {
        ...
        const parsedPayload = this.parseAgentResponse(response.content, role, context.step)
  ...
  private parseAgentResponse(content: string, role: AgentRole, step: string): any {
    try {

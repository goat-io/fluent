...
Multi-provider LLM adapter and multi-agent consensus layer for the Goat workflow engine. Unifies OpenAI, Anthropic, Google, and Ollama behind a single interface, with structured tool-call loops integrated into `@goatlab/agents-core` workflows.
...
- Exposes token/cost accounting so `@goatlab/agents-core` budgets can enforce limits

## Install

```bash
pnpm add @goatlab/agents-ai @goatlab/agents-core
...
```ts
import { createAIAdapter } from '@goatlab/agents-ai'
...
The loop persists each tool invocation through `@goatlab/agents-core`'s external-action machinery when wired to a running workflow — making tool calls exactly-once, replayable, and budget-aware.
...
```ts
import { consensus } from '@goatlab/agents-ai'
...
## Integration with `@goatlab/agents-core`
...
```ts
import { AIStepExecutor } from '@goatlab/agents-ai'
...
| `AIStepExecutor` | Plug into `@goatlab/agents-core` as `executorType: 'ai'` |

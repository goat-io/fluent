...
type RequiredFieldsType<C extends TypesenseCollection> = {
  [F in RequiredFields<C> as Literal<F['name']> extends 'id'
    ? never
    : Literal<F['name']>]: FieldTypeToTS<F['type']>
}
...
type OptionalFieldsType<C extends TypesenseCollection> = {
  [F in OptionalFields<C> as Literal<F['name']> extends 'id'
    ? never
    : Literal<F['name']>]?: FieldTypeToTS<F['type']>
}
 ...
 */
export type InferDocumentType<C extends TypesenseCollection> =
  RequiredFieldsType<C> & OptionalFieldsType<C>

/**
 * Infers the TypeScript type from a collection definition
 */
export type InferFromCollection<C extends TypesenseCollection> =
  InferDocumentType<C>

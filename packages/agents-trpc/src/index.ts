// @goatlab/agents-trpc — drop-in tRPC adapter for the agent engine.
//
...
//
// Usage:
//   import { createAgentsTrpcRouter } from '@goatlab/agents-trpc'
//
...
export interface AgentsTrpcRouterConfig<TProcedure, TInput = unknown> {
  ...
  resolveAgents: (opts: { ctx: TInput }) => Promise<AgentsBundle> | AgentsBundle
  /**
...
export function createAgentsTrpcRouter<TProcedure extends { input: any; query: any; mutation: any }>(
  ...
  const withBundle = async (opts: any, fn: (b: AgentsBundle) => Promise<unknown>) => {
  ...
  if (enabled('startAsync')) {
    procs.startAsync = p
      ...
      .mutation(({ input, ctx }: any) => withBundle({ ctx }, ({ ingestBuffer, tenantId }) => {
        if (!ingestBuffer) throw new Error('startAsync requires resolveAgents to return ingestBuffer')
        const { runId, traceId } = ingestBuffer.enqueue({ ...input, tenantId })
  ...
  if (enabled('start')) {
    procs.start = p
      ...
      // biome-ignore lint/suspicious/noExplicitAny: tRPC opts
      .mutation(({ input, ctx }: any) => withBundle({ ctx }, ({ engine, tenantId }) => {
  ...
  if (enabled('status')) {
    procs.status = p
      ...
      // biome-ignore lint/suspicious/noExplicitAny: tRPC opts
      .query(({ input, ctx }: any) => withBundle({ ctx }, async ({ engine, ingestBuffer, tenantId }) => {
        const handlers = createWorkflowHandlers(engine)
        try {
          ...
          const e = err as any
          if (e?.code === 'WORKFLOW_RUN_NOT_FOUND' && input.runId && ingestBuffer) {
  ...
  if (enabled('query')) {
    procs.query = p
      ...
      // biome-ignore lint/suspicious/noExplicitAny: tRPC opts
      .query(({ input, ctx }: any) => withBundle({ ctx }, ({ engine, tenantId }) => {
        const handlers = createWorkflowHandlers(engine)
  ...
  if (enabled('list')) {
    // biome-ignore lint/suspicious/noExplicitAny: tRPC opts
    procs.list = p.query(({ ctx }: any) => withBundle({ ctx }, ({ engine, tenantId }) => {
      const handlers = createWorkflowHandlers(engine)

...
tRPC adapter for `@goatlab/agents-core`. Mounts the engine as a tRPC router so your client can call `client.workflows.startAsync.mutate({...})` with full type safety.
...
```bash
pnpm add @goatlab/agents-trpc @goatlab/agents-core @trpc/server zod
```
...
import { myFactory } from './my-agents-factory'
...
import { WorkflowEngine, IngestBuffer, IngestWorker } from '@goatlab/agents-core'
...
(Add LRU+TTL eviction for production; see the `agents-express/example` README for the full pattern.)

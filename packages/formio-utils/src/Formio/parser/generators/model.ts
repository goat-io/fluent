...
const generateBaseModel = (
  ...
  Handlebars.registerHelper('stringToJsType', (element, _options) => {
    if (element === 'string') {
      return 'String'
    }
    if (element === 'number') {
  ...
  Handlebars.registerHelper('stringToFaker', (element, array, _options) => {
    if (element === 'string') {
      return array
...
const generateDto = (Model: FluentModel, _framework: SupportedFrameworks) => {

...
if (isBun) {
  ...
  console.log(`[Hono+Bun+SQLite] Server running on http://localhost:${server.port}`)
  console.log(`[Hono+Bun+SQLite] tRPC endpoint: http://localhost:${server.port}/trpc`)
  ...
  console.log('[Hono+Node+SQLite] Note: For accurate benchmarks, run with Bun runtime')

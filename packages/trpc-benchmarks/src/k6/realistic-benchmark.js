...
function trpcQuery(procedure, input) {
	var url = input
		? TRPC_URL +
			...
			procedure +
			"?input=" +
			encodeURIComponent(JSON.stringify(wrapInput(input)))
		: `${TRPC_URL}/${procedure}`;
...
function _getNestedProp(obj, path) {
...
function browseProducts() {
	group("Browse Products", () => {
		...
		var listOk = check(listRes, { "product.list ok": (r) => r.status === 200 });
		...
		if (seenProducts.length > 0) {
			...
			var getOk = check(getRes, { "product.get ok": (r) => r.status === 200 });
			errorRate.add(!getOk);
		}
		...
		var catOk = check(catRes, { "category.list ok": (r) => r.status === 200 });
		errorRate.add(!catOk);
...
function searchProducts() {
	group("Search Products", () => {
		...
		var res = trpcQuery("product.search", {
			query: query,
			page: 1,
			pageSize: 20,
		});
		...
		var ok = check(res, { "product.search ok": (r) => r.status === 200 });
		errorRate.add(!ok);
...
function viewUserAccount() {
	group("User Account", () => {
		var listRes = trpcQuery("user.list", {
		...
		if (seenUsers.length > 0) {
			...
			var userOk = check(userRes, { "user.get ok": (r) => r.status === 200 });
			errorRate.add(!userOk);

			sleep(0.05);

			var ordersRes = trpcQuery("user.orders", {
				userId: userId,
				page: 1,
				pageSize: 10,
			});
			throughput.add(1);
			check(ordersRes, { "user.orders ok": (r) => r.status === 200 });
		}
	});
}
...
function placeOrder() {
	group("Place Order", () => {
		...
		if (seenUsers.length > 0 && seenProducts.length >= 2) {
			...
			if (items.length > 0) {
				var res = trpcMutation("order.create", {
					userId: userId,
					items: items,
				});
				throughput.add(1);
...
function viewOrder() {
	group("View Order", () => {
		...
		if (orderIds.length > 0) {
			...
			var ok = check(res, { "order.get ok": (r) => r.status === 200 });
			errorRate.add(!ok);
		}
	});
}
...
function viewDashboard() {
	group("Dashboard Analytics", () => {
		...
		var ok = check(res, { "analytics.dashboard ok": (r) => r.status === 200 });
		errorRate.add(!ok);

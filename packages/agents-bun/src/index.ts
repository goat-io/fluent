// @goatlab/agents-bun — Bun.serve adapter for the Goat agent engine.
//
...
//
// Usage:
//   import { agentsBunHandler } from '@goatlab/agents-bun'
...
export function agentsBunHandler(config: AgentsBunHandlerConfig) {
  const enabled = (k: keyof NonNullable<AgentsBunHandlerConfig['routes']>): boolean =>
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (path === '/start-async' && enabled('startAsync')) {
        if (!ingestBuffer) throw new Error('start-async requires resolveAgents to return an ingestBuffer')
        const { runId, traceId } = ingestBuffer.enqueue({ ...body, tenantId } as any)
      ...
      if (path === '/start-batch' && enabled('startBatch')) {
        const workflows = ((body.workflows as Array<Record<string, unknown>> | undefined) ?? [])
          .map(w => ({ ...w, tenantId }))
        return json(200, await handlers.startBatch({ workflows: workflows as any }))
      }
      if (path === '/start-batch-copy' && enabled('startBatchCopy')) {
        const workflows = ((body.workflows as Array<Record<string, unknown>> | undefined) ?? [])
          .map(w => ({ ...w, tenantId }))
        return json(200, await handlers.startBatchCopy({ workflows: workflows as any }))
      }
      if (path === '/status' && enabled('status')) {
        try {
          return json(200, await handlers.getStatus({ ...body, tenantId } as any))
        } catch (err) {
      ...
      if (path === '/human-input' && enabled('humanInput')) {
        return json(200, await handlers.submitHumanInput({ ...body, tenantId } as any))
      }
      ...
      if (path === '/ingest-event' && enabled('ingestEvent')) {
        return json(200, await handlers.ingestEvent({ ...body, tenantId } as any))
      }

...
[Bun](https://bun.sh) adapter for `@goatlab/agents-core`. Returns a single `fetch(req)` handler that mounts every workflow endpoint behind `Bun.serve()`.
...
```bash
bun add @goatlab/agents-bun @goatlab/agents-core
```
...
import { myAgentsFactory } from './my-agents-factory'
...
This package is intentionally tiny (~200 LOC) — the heavy lifting lives in `@goatlab/agents-core`.

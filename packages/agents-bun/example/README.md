...
A blank Bun app that mounts the Goat agent engine via [`@goatlab/agents-bun`](..). Mirror of the [Express example](../../agents-express/example) — same Prisma schema, same engine factory pattern, same load-test script — so you can A/B compare runtimes apples-to-apples.
...
# 1. From this folder:
cd packages/agents-bun/example

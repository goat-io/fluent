// Bun server hosting the Goat agent engine via @goatlab/agents-bun.
//
...
async function runServer() {
  ...
  await getAgents()
  console.log(`[${process.env.WORKER_ID ?? 'single'}] ✅ engine + workers ready`)
  ...
  console.log(`[${process.env.WORKER_ID ?? 'single'}] ✅ Bun + agent engine on http://localhost:${PORT}`)
}
...
if (isWorker || N === 1) {
  ...
  console.log(`🧠 Primary pid=${process.pid} — spawning ${N} Bun workers (CLUSTER_MODE=${process.env.WORKER_ID ?? 'auto'})`)
  const children: ReturnType<typeof spawn>[] = []
  for (let i = 0; i < N; i++) {
    const child = spawn(process.execPath, ['run', new URL(import.meta.url).pathname], {

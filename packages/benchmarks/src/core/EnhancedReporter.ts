...
export class EnhancedBenchmarkReporter {
  ...
  formatTime(ms: number): string {
    if (ms < 1) return chalk.cyan(`${(ms * 1000).toFixed(2)}μs`)
    if (ms < 1000) return chalk.cyan(`${ms.toFixed(2)}ms`)
    return chalk.cyan(`${(ms / 1000).toFixed(2)}s`)
  }

  formatMemory(bytes: number): string {
    if (bytes < 1024) return chalk.magenta(`${bytes}B`)
    if (bytes < 1024 * 1024) return chalk.magenta(`${(bytes / 1024).toFixed(2)}KB`)
    if (bytes < 1024 * 1024 * 1024) return chalk.magenta(`${(bytes / 1024 / 1024).toFixed(2)}MB`)
    return chalk.magenta(`${(bytes / 1024 / 1024 / 1024).toFixed(2)}GB`)
  }

  formatOpsPerSecond(ops: number): string {
    if (ops < 1000) return chalk.green(`${ops.toFixed(2)} ops/s`)
    if (ops < 1000000) return chalk.green(`${(ops / 1000).toFixed(2)}k ops/s`)
  ...
  printPerformanceInsights(results: BenchmarkResult[]): void {
    ...
    const sortedResults = [...results].sort((a, b) => b.operationsPerSecond - a.operationsPerSecond)
    const winner = sortedResults[0]
    const runnerUp = sortedResults[1]
    
    // Calculate advantages
    const speedAdvantage = ((winner.operationsPerSecond / runnerUp.operationsPerSecond) - 1) * 100
    const memoryAdvantage = ((runnerUp.memoryUsage.heapUsed / winner.memoryUsage.heapUsed) - 1) * 100
    
    // Generate grade based on performance difference
    const getGrade = (advantage: number) => {
      if (advantage > 100) return 'A+'
      if (advantage > 50) return 'A'
      if (advantage > 25) return 'B'
      if (advantage > 10) return 'C'
      return 'D'
    }
    ...
    if (results.length > 2) {
      content.push(`🥉 Third: ${chalk.grey(sortedResults[2].name.split(' - ')[0])}`)
      if (results.length > 3) {
        content.push(`🏅 Fourth: ${chalk.grey(sortedResults[3].name.split(' - ')[0])}`)
      }
    }
  ...
  printSummaryStats(results: BenchmarkResult[]): void {
    ...
    const activeDrivers = drivers.filter(driver => results.some(r => r.name.includes(driver)))
    const content = [
      chalk.bold.yellow('📈 Summary Statistics'),
      '',
      ...activeDrivers.map((driver, index) => {
        const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : '🏅'
        const color = index === 0 ? 'green' : index === 1 ? 'blue' : index === 2 ? 'yellow' : 'magenta'
  ...
  printRecommendations(results: BenchmarkResult[]): void {
    ...
    if (winner.name === 'MySQL2') {
      recommendations.push('🚀 For maximum performance, MySQL2 provides direct database access')
      recommendations.push('📊 MySQL2 shows consistent speed advantages for raw queries')
    } else if (winner.name === 'Kysely') {
      recommendations.push('⚡ Kysely offers excellent type safety with minimal performance overhead')
      recommendations.push('🛠️ Consider Kysely for type-safe queries without ORM complexity')
    } else if (winner.name === 'Drizzle') {
      recommendations.push('🔥 Drizzle provides modern TypeScript experience with great performance')
      recommendations.push('📈 Drizzle balances developer experience with runtime efficiency')
    } else if (winner.name === 'Prisma') {
      recommendations.push('✨ Prisma excels in developer experience with competitive performance')
      recommendations.push('🛡️ Consider Prisma for full-featured ORM with excellent tooling')
    }

    if (secondBest && (winner.avgOps / secondBest.avgOps) < 1.2) {
      recommendations.push(`🤝 ${winner.name} and ${secondBest.name} show similar performance - choose based on your needs`)
    }

    recommendations.push('🔄 For read-heavy workloads, consider raw SQL approaches (MySQL2/Kysely)')
    recommendations.push('✨ For complex applications, ORMs (Prisma/Drizzle) provide better abstractions')
    recommendations.push('📈 Consider mixing approaches: ORM for CRUD, raw queries for complex operations')
  ...
  exportToJson(suite: BenchmarkSuite, filename?: string): void {
    ...
    console.log(boxen(
      `📁 Results exported to ${chalk.green(fileName)}`,
      {
        padding: 1,
        borderStyle: 'round',
        borderColor: 'green',
  ...
  printEnvironmentInfo(): void {
    console.log(boxen([
      ...
      `CPU Count: ${chalk.green(require('os').cpus().length)}`,
      `Memory: ${this.formatMemory(require('os').totalmem())}`,

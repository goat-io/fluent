...
export class ComparisonEngine {
   ...
   */
  compareResults(result1: BenchmarkResult, result2: BenchmarkResult): ComparisonResult {
    const speedRatio = result1.operationsPerSecond / result2.operationsPerSecond
    const memoryRatio = result2.memoryUsage.heapUsed / result1.memoryUsage.heapUsed
    ...
    const recommendation = this.generateRecommendation(speedAdvantage, memoryAdvantage, winner)
  ...
  private renderScoreCards(scoreCards: ScoreCard[]): void {
    ...
    scoreCards.forEach(card => {
      const cardColor = card.overall >= 85 ? 'green' : card.overall >= 70 ? 'yellow' : 'red'
  ...
  private renderHeadToHeadComparisons(results: BenchmarkResult[]): void {
    ...
    scenarios.forEach(scenario => {
      const mysql2Result = results.find(r => r.name.includes('MySQL2') && r.name.includes(scenario))
      const prismaResult = results.find(r => r.name.includes('Prisma') && r.name.includes(scenario))
      
      if (mysql2Result && prismaResult) {
        const comparison = this.compareResults(mysql2Result, prismaResult)
        this.renderScenarioComparison(scenario, comparison, mysql2Result, prismaResult)
      }
    })
  }
  ...
  private renderDecisionMatrix(results: BenchmarkResult[]): void {
    ...
    console.log(boxen([
      ...
      margin: 1,
      borderStyle: 'double',
      borderColor: mysql2Wins > prismaWins ? 'green' : prismaWins > mysql2Wins ? 'blue' : 'yellow',
      width: 50
    }))
  }
  ...
  private renderFinalVerdict(scoreCards: ScoreCard[]): void {
    ...
    if (!mysql2Card || !prismaCard) return
    
    const winner = mysql2Card.overall > prismaCard.overall ? mysql2Card : prismaCard
    const margin = Math.abs(mysql2Card.overall - prismaCard.overall)
    
    const verdict = this.generateVerdict(winner, margin)
    const recommendation = this.generateFinalRecommendation(mysql2Card, prismaCard)
   ...
   */
  private calculateGrade(score: number): 'A+' | 'A' | 'B+' | 'B' | 'C+' | 'C' | 'D' {
   ...
   */
  private calculateLibraryScore(libraryResults: BenchmarkResult[], allResults: BenchmarkResult[]): {
    speed: number
    memory: number
    consistency: number
    overall: number
    ...
    const avgOps = libraryResults.reduce((sum, r) => sum + r.operationsPerSecond, 0) / libraryResults.length
    const avgMemory = libraryResults.reduce((sum, r) => sum + r.memoryUsage.heapUsed, 0) / libraryResults.length
    
    // Calculate consistency (lower variance = higher consistency)
    const opsVariance = this.calculateVariance(libraryResults.map(r => r.operationsPerSecond))
  ...
  private countWins(results1: BenchmarkResult[], results2: BenchmarkResult[]): number {
    let wins = 0
    results1.forEach(r1 => {
  ...
  private generateFinalRecommendation(mysql2: ScoreCard, prisma: ScoreCard): string {
    ...
    } else if (winner.memory > loser.memory + 15) {

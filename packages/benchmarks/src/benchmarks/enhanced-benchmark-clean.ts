...
export class CleanEnhancedBenchmark {
  ...
  async run(): Promise<void> {
    try {
      ...
      console.log(chalk.gray(`Think time: ${this.APPLY_THINK_TIME ? 'ENABLED (realistic user simulation)' : 'DISABLED (pure throughput)'}\n`))
      ...
      const drivers = [
        ...
        'Kysely', 
        'Drizzle'
      ] // Start with core drivers that work reliably
      ...
      for (const workload of workloads) {
        ...
        for (const driver of drivers) {
          ...
          try {
            ...
            const result = await this.benchmarkDriver(workload, driver, connection)
            results.push(result)
            
            console.log(chalk.green(`✓ ${result.totalOperations} ops @ ${result.operationsPerSecond.toFixed(0)} ops/sec`))
          } catch (error) {
  ...
  private async benchmarkDriver(
    ...
    while (Date.now() < measurementEnd) {
      ...
      try {
        ...
        if (this.APPLY_THINK_TIME) {
          const thinkingTime = this.getThinkTime(workload.thinkTime.thinkingTime)
  ...
  private displayResults(workloadName: string, results: DriverResult[]): void {
    ...
    sorted.forEach((result, index) => {
      ...
      const errorRate = result.totalOperations > 0 
  ...
  private displayTransactionBreakdown(results: DriverResult[]): void {
    ...
    console.log(chalk.gray('Shows how each driver handled different transaction types'))
    console.log(chalk.gray('─'.repeat(85)))
    ...
    // Header
    console.log('Transaction'.padEnd(20) + results.map(r => r.driver.padStart(12)).join(''))
    console.log(chalk.gray('─'.repeat(85)))
    ...
    txTypes.forEach(txName => {
      const values = results.map(r => {
        ...
        if (!stats || stats.count === 0) return '0'.padStart(12)
  ...
  private getTransactionExecutors(driver: string, connection: any): Record<string, () => Promise<void>> {
    ...
    switch (driver) {
      ...
      case 'Kysely':
        return {
          simpleSelect: async () => {
            await connection.selectFrom('users').where('id', '=', userId()).selectAll().executeTakeFirst()
          },
          filteredSelect: async () => {
      ...
      case 'Drizzle':
        return {
          simpleSelect: async () => {
            await connection.select().from(schema.users).where(eq(schema.users.id, userId()))
          },
          filteredSelect: async () => {
      ...
      case 'Prisma+Kysely':
        return {
          simpleSelect: async () => {
            await connection.$kysely.selectFrom('users').where('id', '=', userId()).selectAll().executeTakeFirst()
          },
          filteredSelect: async () => {
            await connection.$kysely.selectFrom('users')
              .where('status', '=', 'active')
              .where('age', '>', age())
              .selectAll()
              .limit(50)
          ...
          joinQuery: async () => {
            await connection.$kysely.selectFrom('users as u')
              .leftJoin('orders as o', 'u.id', 'o.user_id')
              .select(['u.id', 'u.email', 'u.first_name', 'u.last_name'])
          ...
          complexJoin: async () => {
            await connection.$kysely.selectFrom('products as p')
              .leftJoin('categories as c', 'p.category_id', 'c.id')
              .leftJoin('reviews as r', 'p.id', 'r.product_id')
              .select(['p.id', 'p.name', 'p.price'])
              .select('c.name as category_name')
      ...
      case 'TypeORM':
        return {
          ...
          complexJoin: async () => {
            ...
            await connection.getRepository(Product)

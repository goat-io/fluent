...
async function main() {
  ...
  let queryCount = { mysql2: 0, prisma: 0, kysely: 0, drizzle: 0 }
  ...
  for (const scenario of scenarios) {
    ...
    console.log('\nPerformance results:')
    console.log(`MySQL2:  ${mysql2Result.operationsPerSecond.toFixed(0)} ops/sec, ${mysql2Result.averageTime.toFixed(2)}ms avg`)
    console.log(`Prisma:  ${prismaResult.operationsPerSecond.toFixed(0)} ops/sec, ${prismaResult.averageTime.toFixed(2)}ms avg`)
    console.log(`Kysely:  ${kyselyResult.operationsPerSecond.toFixed(0)} ops/sec, ${kyselyResult.averageTime.toFixed(2)}ms avg`)
    console.log(`Drizzle: ${drizzleResult.operationsPerSecond.toFixed(0)} ops/sec, ${drizzleResult.averageTime.toFixed(2)}ms avg`)
  }
  ...
  const times: Record<string, number[]> = { mysql2: [], prisma: [], kysely: [], drizzle: [] }
  ...
  for (const [driver, driverTimes] of Object.entries(times)) {
    ...
    const improvement = ((first10Avg - last10Avg) / first10Avg * 100).toFixed(1)
    
    console.log(`${driver.padEnd(8)}: First 10 avg=${first10Avg.toFixed(2)}ms, Last 10 avg=${last10Avg.toFixed(2)}ms, Improvement=${improvement}%`)
  }

...
export class PerformanceChart {
   ...
   */
  renderBarChart(results: BenchmarkResult[], metric: 'operationsPerSecond' | 'averageTime' | 'memoryUsage'): void {
    ...
    results.forEach((result, index) => {
      ...
      console.log(`${chalk.bold(label)} ${coloredBar} ${chalk.yellow(valueStr)}`)
    })
  ...
  renderTrendChart(results: BenchmarkResult[]): void {
    ...
    for (let row = this.CHART_HEIGHT; row >= 0; row--) {
      ...
      const yLabel = this.formatValue(threshold, 'operationsPerSecond').padStart(8)
    ...
    // X-axis labels
    let xAxis = ' '.repeat(10) + '└' + '─'.repeat(8 * values.length)
  ...
  renderHistogram(results: BenchmarkResult[]): void {
    ...
    results.forEach(result => {
      ...
      times.forEach(time => {
        const bucketIndex = Math.min(Math.floor((time - minTime) / bucketSize), 9)
        buckets[bucketIndex]++
      })
    })
    ...
    buckets.forEach((count, index) => {
      ...
      console.log(`${bucketLabels[index].padEnd(20)} ${coloredBar} ${chalk.yellow(count.toString())}`)
    })
  ...
  renderComparisonMatrix(results: BenchmarkResult[]): void {
    ...
    if (mysql2Results.length === 0 || prismaResults.length === 0) return

    // Create comparison matrix
    const scenarios = [...new Set(results.map(r => r.name.split(' - ')[1]))]
    
    console.log(`${'Scenario'.padEnd(20)} │ ${'MySQL2'.padEnd(12)} │ ${'Prisma'.padEnd(12)} │ ${'Winner'.padEnd(10)} │ Advantage`)
  ...
  renderMemoryChart(results: BenchmarkResult[]): void {
    ...
    results.forEach(result => {
      ...
      console.log(`${result.name.padEnd(25)} ${coloredBar} ${chalk.magenta(memoryMB.toFixed(2) + ' MB')}`)
    })
  ...
  private colorizeBar(bar: string, normalizedValue: number): string {
    if (normalizedValue > 0.8) return chalk.green(bar)
    if (normalizedValue > 0.6) return chalk.yellow(bar)
    if (normalizedValue > 0.4) return chalk.red(bar)
    return chalk.grey(bar)
  }

  private colorizeMemoryBar(bar: string, normalizedValue: number): string {
    if (normalizedValue > 0.8) return chalk.red(bar)      // High memory usage
    if (normalizedValue > 0.6) return chalk.yellow(bar)   // Medium memory usage

...
const search = async (lbQueryUrl: any) =>
  await Submission({ path: lbQueryUrl.path })
  ...
  staticRequest: async (request, options) => {
    ...
    if (url.includes('&where')) {
      where = decodeURIComponent(
        url.substring(url.lastIndexOf('&where') + 6, url.lastIndexOf('&filter'))
      ).replace('=', '')
    ...
    try {
      if (lbQueryUrl.where) {
        ...
        lbQueryUrl.where = {
          id: Array.isArray(currentValue) ? { inq: currentValue } : currentValue
        }
        lbQueryUrl.filter.where = lbQueryUrl.where
      }
    } catch (error) {

...
export class LocalTime {
  ...
  static earliest(items: LocalTimeConfig[]): LocalTime {
    assert(items.length, 'LocalTime.earliest called on empty array')
  ...
  static latest(items: LocalTimeConfig[]): LocalTime {
    assert(items.length, 'LocalTime.latest called on empty array')
...
function differenceInMonths(a: Date, b: Date): number {
  ...
  return -(
    wholeMonthDiff +
    ((b.getTime() - anchor) / (anchor2 - anchor)) * sign
  )
}

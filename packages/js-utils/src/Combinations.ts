...
class _CBase<T, U> {
   ...
   */
  sample(): Optional<U[]> {
    return this.nth(combinatory.randomInteger(this.length))
  }
...
class Combinatory {
   ...
   */
  combinadic(n: number, k: number) {
    const count = this.combination(n, k)
    return (m: anyint): number[] => {
      ...
      for (let i = 0; i < k; i++) {
        a--
        while (x < this.combination(a, b)) a--
        digits.push(n - 1 - a)
        x -= _BI(this.combination(a, b))
        b--
      }
      return digits
    }
   ...
   * @param {anyint} max
   */
  randomInteger(min: anyint = 0, max: anyint = 2 ** 53) {
...
const combinatory = new Combinatory()
/**
 * Permutation
 */
export class Permutation<T> extends _CBase<T, T> {
  constructor(seed: Iterable<T>, size = 0) {
    ...
    this.length = combinatory.permutation(this.seed.length, this.size)
    Object.freeze(this)
  }
  ...
  nth(n: anyint): Optional<T[]> {
    ...
    const skip = combinatory.factorial(offset)
    const digits = combinatory.factoradic(_BI(n) * _BI(skip), this.seed.length)
...
export class Combination<T> extends _CBase<T, T> {
  ...
  constructor(seed: Iterable<T>, size = 0) {
    ...
    this.size = size > 0 ? size : this.seed.length
    this.size = size
    this.length = combinatory.combination(this.seed.length, this.size)
    this.comb = combinatory.combinadic(this.seed.length, this.size)
    Object.freeze(this)
  }

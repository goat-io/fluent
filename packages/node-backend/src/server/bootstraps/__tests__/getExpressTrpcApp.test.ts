...
describe('getExpressTrpcApp configuration', () => {
  describe('getDefaultConfig', () => {
    it('should work with minimal configuration (just trpcRouter)', () => {
      ...
      expect(result.security?.cors?.allowedOrigins).toEqual(['http://localhost:3000', 'http://localhost:3001'])
    ...
    it('should allow arrays to be completely replaced', () => {
      ...
      expect(result.security?.cors?.allowedOrigins).toEqual(['https://example.com', 'https://app.example.com'])
      expect(result.security?.cors?.allowedOrigins).not.toContain('http://localhost:3000')
    })
    ...
    it('should apply production defaults when environment is prod', () => {
      ...
      expect(result.performance?.memoryMonitoring?.enableGarbageCollection).toBe(true)

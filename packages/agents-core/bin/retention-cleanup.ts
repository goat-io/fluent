 ...
 *   apiVersion: batch/v1
 *   kind: CronJob
 *   metadata: { name: agents-retention }
 *   spec:
 *     schedule: "0 * * * *"
 *     jobTemplate:
 *       spec:
...
async function main() {
  ...
  console.log(`[retention] cutoff = ${RETENTION_DAYS}d, batch = ${BATCH_SIZE}`)

...
export class TaskRunnerExecutor implements StepExecutor {
  ...
  async execute(payload: StepPayload, context?: StepExecutionContext): Promise<StepResult> {
    ...
    if (!taskManager) {
      throw new Error('TaskRunnerExecutor requires taskManager in StepExecutionContext')
    }
    ...
    if (!innerExecutor) {
      throw new Error(`No executor registered for inner type "${innerExecutorType}"`)
    }
    ...
    while (true) {
      ...
      if (checkBudget) {
        const exceeded = await checkBudget(payload.workflowRunId, 'taskExecutions')
        if (exceeded) {
      ...
      const canRun = await taskManager.checkTaskConcurrency(payload.workflowRunId, maxConcurrentTasks)
      if (!canRun) {

...
export interface GitHubClient {
  createPR(req: { title: string; body: string; branch: string; baseBranch?: string }): Promise<{ id: string; number: number; url: string }>
  createIssue(req: { title: string; body: string; labels?: string[] }): Promise<{ id: string; number: number; url: string }>
  addComment(req: { issueNumber: number; body: string }): Promise<{ id: string }>
  mergePR(req: { prNumber: number; mergeMethod?: string }): Promise<{ merged: boolean; sha: string }>
}

...
describe('Budget Guardrails', () => {
  ...
  beforeEach(async () => {
    ...
    fnExecutor.register('echo', async (payload: StepPayload): Promise<StepResult> => {
      return { output: { echoed: true, step: payload.stepName } }
    })
  })

  function createEngine(budget?: {
    maxSteps?: number; maxTokens?: number; maxCostUsd?: number; maxTaskExecutions?: number
  }) {
    ...
    const wf = WorkflowBuilder.create('budget-wf')
      ...
      .step('step3', { executorType: 'function', executorConfig: { handler: 'echo' }, dependsOn: ['step2'] })
      .build()
  ...
  it('budget is initialized to zero on workflow start', async () => {
    ...
    const { runId } = await engine.start({
      workflowName: 'budget-wf', tenantId: 'test-tenant', input: {},
    })

    const run = await db.selectFrom('workflow_runs')
      .selectAll().where('id', '=', runId).executeTakeFirstOrThrow()

    const budgetUsed = fromJson<any>(run.budgetUsed)
    expect(budgetUsed).toEqual({ tokens: 0, costUsd: 0, steps: 0, taskExecutions: 0 })
  ...
  it('step counter increments after each completed step', async () => {
    ...
    let run = await db.selectFrom('workflow_runs')
      .selectAll().where('id', '=', runId).executeTakeFirstOrThrow()
    ...
    run = await db.selectFrom('workflow_runs')
      .selectAll().where('id', '=', runId).executeTakeFirstOrThrow()
  ...
  it('tracks token and cost usage from step output _usage field', async () => {
    fnExecutor.register('ai_step', async (): Promise<StepResult> => ({
      output: {
    ...
    const wf = WorkflowBuilder.create('cost-wf')
      .step('ai', { executorType: 'function', executorConfig: { handler: 'ai_step' } })
      .step('next', { executorType: 'function', executorConfig: { handler: 'echo' }, dependsOn: ['ai'] })
      .build()
    ...
    const { runId } = await engine.start({
      workflowName: 'cost-wf', tenantId: 'test-tenant', input: {},
    })
  ...
  it('maxTokens exceeded fails workflow mid-execution', async () => {
    fnExecutor.register('expensive', async (): Promise<StepResult> => ({
      output: {
        data: 'expensive output',
        _usage: { tokens: 5000, costUsd: 0.50 },
      },
    }))

    const { connector, queuedJobs } = createMockConnector()
    const wf = WorkflowBuilder.create('token-wf')
      .step('s1', { executorType: 'function', executorConfig: { handler: 'expensive' } })
      .step('s2', { executorType: 'function', executorConfig: { handler: 'echo' }, dependsOn: ['s1'] })
      .build()
    ...
    const { runId } = await engine.start({
      workflowName: 'token-wf', tenantId: 'test-tenant', input: {},
    })
  ...
  it('maxTaskExecutions stops task_runner before processing all tasks', async () => {
    ...
    fnExecutor.register('create_tasks', async (p: StepPayload, ctx?: StepExecutionContext): Promise<StepResult> => {
    ...
    const wf = WorkflowBuilder.create('task-budget-wf')
      .step('create', { executorType: 'function', executorConfig: { handler: 'create_tasks' } })
      .step('run_tasks', {
        dependsOn: ['create'],
        executorType: 'task_runner',
        executorConfig: { executor: 'function', handler: 'process', maxConcurrentTasks: 10 },
      })
      .build()

    const engine = new WorkflowEngine({
      db, connector,
      executors: new Map<string, any>([['function', fnExecutor], ['task_runner', taskRunnerExecutor]]),
      workflows: new Map([[wf.name, wf]]),
      tenantId: 'test-tenant',
      disableLogBuffering: true,
    ...
    const { runId } = await engine.start({
      workflowName: 'task-budget-wf', tenantId: 'test-tenant', input: {},
    })
    ...
    const ctx: StepExecutionContext = {
      ...
      checkBudget: (rid, field, amount) => engine.incrementBudgetUsage(rid, field as any, amount),
    }
    const createResult = await fnExecutor.execute(createPayload, ctx)
    await engine.onStepCompleted(createPayload.workflowRunId, createPayload.stepName, createPayload.tenantId, createResult)
    ...
    await engine.onStepCompleted(runPayload.workflowRunId, runPayload.stepName, runPayload.tenantId, runResult)

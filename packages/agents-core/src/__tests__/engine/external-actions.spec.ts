...
import { ExternalActionExecutor, ExternalActionPendingError } from '../../engine/ExternalActionExecutor.js'
...
describe('ExternalActionExecutor', () => {
  ...
  describe('exactly-once execution', () => {
    ...
    it('returns cached result on duplicate call (same idempotency key)', async () => {
      let callCount = 0
      const fn = async () => { callCount++; return { externalId: 'LIN-1', data: { created: true } } }
    ...
    it('different idempotency keys execute separately', async () => {
      let callCount = 0
      const fn = async () => { callCount++; return { externalId: `LIN-${callCount}`, data: {} } }
    ...
    it('custom idempotency key overrides default', async () => {
      ...
      const second = await executor.execute(
        { ...baseReq, idempotencyKey: 'custom-key-123', request: { different: true } },
  ...
  describe('failure handling', () => {
    ...
    it('failed action can be retried (new attempt)', async () => {
      // First attempt fails
      await expect(
        executor.execute(baseReq, async () => { throw new Error('fail') }),
      ).rejects.toThrow()

      // Second attempt with different key succeeds
      const result = await executor.execute(
        { ...baseReq, attempt: 2, idempotencyKey: 'wf-1:create_tasks:create_issue:2' },
  ...
  describe('audit trail', () => {
    it('stores request and response payloads', async () => {
      await executor.execute(
        ...
        async () => ({ externalId: 'LIN-99', data: { url: 'https://linear.app/issue/LIN-99' } }),
      )
      ...
      expect(JSON.parse(action.request!)).toEqual({ title: 'My Issue', labels: ['bug'] })
      expect(JSON.parse(action.response!)).toEqual({ url: 'https://linear.app/issue/LIN-99' })

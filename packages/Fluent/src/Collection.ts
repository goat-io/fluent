...
export const Collection = stampit({
  init(data) {
  ...
  methods: {
    ...
    average(path = '') {
      let data = [...this.data]
      const sum = data.reduce((acc: number, element) => {
        let value = element

        if (element instanceof Object) {
          let extract = Objects().getFromPath(element, path, undefined)
    ...
    async chunkApply(size: number, callback: (d: any) => void) {
    ...
    contains(...args) {
      ...
      return data.some((e, index) => {
        ...
        let val = e
        if (e instanceof Object) {
          let extract = Objects().getFromPath(e, path, undefined)
          if (extract.value) {
            val = extract.value
          }
        }
    ...
    duplicatesBy(keys) {
      ...
      data.reduce((object, submission) => {
        const finalKey = keys.reduce((string, key) => string + Objects().getFromPath(submission, key, '').value, '')

...
export const Loopback = Interface.compose({
  methods: {
    getToken() {
    ...
    async get() {
      ...
      if (result && result.data && result.data.hasOwnProperty('meta') && this.selectArray.length > 0) {
    ...
    getUrl() {
      const baseUrl = this && this.baseUrl() ? this.baseUrl() : undefined
      const path =
        Objects().get(() => this.path, undefined) || Objects().get(() => this.remoteConnection.path, undefined)

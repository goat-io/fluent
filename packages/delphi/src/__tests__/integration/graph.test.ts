      ...
      const mockClients = [
        {
          ...
          json: vi.fn().mockResolvedValueOnce({ draft: 'Create or update README.md file with touch command' }),
        },
        {
          post: vi.fn().mockReturnThis(),
          json: vi.fn().mockResolvedValueOnce({ refined: 'Execute: touch README.md in the project root', clear: true }),
        },
        {
          post: vi.fn().mockReturnThis(),
          json: vi.fn().mockResolvedValueOnce({ ok: true, feedback: '✅ Approved' }),
        },
      ];
      
      let clientIndex = 0;
      vi.mocked(Http.getClient).mockImplementation(() => mockClients[clientIndex++] as any);
  ...
  describe('IT-persist-crash: Resume from checkpoint', () => {
    it('should resume from last Redis checkpoint after crash', async () => {
      ...
      const mockClients = [
        {
          post: vi.fn().mockReturnThis(),
          json: vi.fn().mockResolvedValueOnce({ spec: 'Create a new README.md file' }),
        },
        {
          post: vi.fn().mockReturnThis(),
          json: vi.fn().mockResolvedValueOnce({ refined: 'Create README.md with project info CLEAR: TRUE', clear: true }),
        },
        {
          post: vi.fn().mockReturnThis(),
          json: vi.fn().mockResolvedValueOnce({ ok: true, feedback: '✅ Approved' }),

...
const dbPath = process.env.SQLITE_DB_PATH || path.join(process.cwd(), ".delphi", "checkpoints.db");
...
export async function cleanupOldCheckpoints(daysToKeep: number = 7): Promise<void> {
  try {
    ...
    console.log(`🧹 Cleanup of checkpoints older than ${daysToKeep} days would be performed here`);
    
    // Run VACUUM to reclaim space (should be done periodically)
    if (Math.random() < 0.1) { // 10% chance to run VACUUM

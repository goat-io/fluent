...
import { Annotation, createGraph } from "@langchain/langgraph";
...
import {
	...
	parseConsensusResult,
} from "../src/agreement/types";
 ...
 */
async function runArchitectureExample(
	goal?: string,
): Promise<ConsensusResult | null> {
	const traceId = crypto.randomUUID();
	...
	try {
		...
		const discussion = new DiscussionBuilder<ConsensusResult>()
			.goal(
				goal || "Choose cloud provider and architecture for new SaaS platform",
			)
			.withConstraints(
...
function processUserData(userData) {
  ...
  `;
	...
	const discussion = refactoringDiscussion()
		...
		.expecting("Refactored code", "List of improvements made", "Test cases")
		.withTimeout(120000)
		.configure({
			maxTurns: 3,
			maxDurationMs: 90000,

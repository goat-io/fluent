...
const GoatExportUtilities = {
  ...
  verifyProperties (obj, props) {
    ...
    _.forEach(props, (prop, key) => {
      ...
      if (prop.required && _.isNil(obj[key])) {
        ...
        throw new Error(`[GoatExportUtilities] invalid property (${key} is required)`);
      }
      ...
      if (prop.hasOwnProperty('type') && obj.hasOwnProperty(key)) {
        ...
        let found = !!_.find(_.isArray(prop.type) ? prop.type : [prop.type], (type) => {
          return GoatExportUtilities.isOfType(obj[key], type);
        });

        // check if there has been a match
        if (!found) {
          // if no match found, throw error
          throw new Error(`[GoatExportUtilities] invalid property (${key} type is invalid)`);
        }
      }
    });
  ...
  isOfType (obj, type) {
    switch (type) {
      ...
      default:
        console.warn('[GoatExportUtilities] type not implemented');
        return false;
  ...
  getValue (data, key) {
    ...
    return GoatExportUtilities.isFormioSubmission(data) ? search(data.data) : search(data);
  },
      ...
      if (!noRecurse) {
        if (hasColumns) {
          component.columns.forEach((column) =>
            GoatExportUtilities.eachComponent(column.components, fn, includeAll, subPath(), parent ? component : null));
        } else if (hasRows) {
          component.rows.forEach((row) => row.forEach((column) =>
            GoatExportUtilities.eachComponent(column.components, fn, includeAll, subPath(), parent ? component : null)));
        } else if (hasComps) {
          GoatExportUtilities.eachComponent(component.components, fn, includeAll, subPath(), parent ? component : null);
        }
      }
    });
  },
...
export default GoatExportUtilities;

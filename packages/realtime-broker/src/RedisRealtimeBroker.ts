...
export interface RedisRealtimeBrokerConfig {
   ...
   */
  perTenantCredentials?: (tenantId: string) => Promise<Partial<RedisOptions> | null>
  /**
...
class TenantSubscriber {
  ...
  constructor(redis: Redis) {
    ...
    redis.on('message', (channel: string, payload: string) => {
      ...
      for (const h of handlers) {
        try { h(parsed, channel) } catch { /* swallow — don't let one bad handler break others */ }
      }
    })
  }
  ...
  isEmpty(): boolean { return this.channels.size === 0 }
...
export class RedisRealtimeBroker implements RealtimeBroker {
  ...
  private async getTenantSubscriber(tenantId: string): Promise<TenantSubscriber> {
  ...
  async publish(tenantId: string, channel: string, data: unknown): Promise<number> {
    if (this.closed) throw new Error('RedisRealtimeBroker is closed')
    if (!this.publisher) {
  ...
  async close(): Promise<void> {
    ...
    if (this.publisher) {
      closes.push(this.publisher.quit().then(() => undefined).catch(() => undefined))

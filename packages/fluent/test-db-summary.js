...
databases.forEach(db => {
  ...
  try {
    ...
    try {
      ...
      const passed = jsonResult.testResults[0].assertionResults.filter(
        r => r.status === 'passed'
      ).length
      const failed = jsonResult.testResults[0].assertionResults.filter(
        r => r.status === 'failed'
      ).length
      ...
      console.log(
        `✅ ${passed}/${total} tests passing (${Math.round((passed / total) * 100)}%)`
      )
    } catch (_parseError) {
      // Fallback for non-JSON output
      const match = output.match(/(\d+) failed.*?(\d+) passed.*?\((\d+)\)/)
      if (match) {
        const failed = Number.parseInt(match[1]) || 0
        const passed = Number.parseInt(match[2]) || 0
        const total = Number.parseInt(match[3]) || 0

...
Workflow dashboard for `@goatlab/agents-core`. A React + React Flow + Vite app that visualizes runs, events, workers, and metrics in real time via server-sent events. Ships with a Node test server used for Playwright E2E and k6 load benchmarks.
...
```bash
pnpm add @goatlab/agents-ui
```

Dashboard components are exported from the package root; styles from `@goatlab/agents-ui/styles.css`.
...
Load-tested with [`k6-workflow.js`](../agents-core/loadtest/k6-workflow.js):

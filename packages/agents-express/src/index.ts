// @goatlab/agents-express — drop-in Express adapter for the Goat agent engine.
//
...
export type AgentsResolver = (req: Request) => Promise<AgentsBundle> | AgentsBundle
...
const defaultErrorMap = (err: unknown): { status: number; body: object } => {
  ...
  if (code === 'WORKFLOW_RUN_NOT_FOUND') return { status: 404, body: { error: message, code } }
  if (code === 'IDEMPOTENCY_CONFLICT')   return { status: 409, body: { error: message, code } }
...
export function agentsRouter(config: AgentsRouterConfig): Router {
  const r = Router()
  const enabled = (k: keyof NonNullable<AgentsRouterConfig['routes']>): boolean =>
    config.routes?.[k] ?? true
  const mapError = config.mapError ?? defaultErrorMap

  // Helper: wraps an async handler with the resolveAgents call + error mapping
  const wrap = (
    fn: (bundle: AgentsBundle, req: Request, res: Response) => Promise<unknown>,
  ) =>
  ...
  if (enabled('startAsync')) {
    r.post('/start-async', wrap(async ({ ingestBuffer, tenantId }, req) => {
      if (!ingestBuffer) {
        throw new Error('start-async requires resolveAgents to return an ingestBuffer')
      }
  ...
  if (enabled('start')) {
    r.post('/start', wrap(async ({ engine, tenantId }, req) => {
  ...
  if (enabled('startBatch')) {
    r.post('/start-batch', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
      const workflows = ((req.body?.workflows ?? []) as Array<Record<string, unknown>>)
        .map(w => ({ ...w, tenantId }))
  ...
  if (enabled('startBatchCopy')) {
    r.post('/start-batch-copy', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
      const workflows = ((req.body?.workflows ?? []) as Array<Record<string, unknown>>)
        .map(w => ({ ...w, tenantId }))
  ...
  if (enabled('status')) {
    r.post('/status', wrap(async ({ engine, ingestBuffer, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
      try {
        ...
        if (e?.code === 'WORKFLOW_RUN_NOT_FOUND' && runId && ingestBuffer) {
          const queue = (engine as any).config?.connector?.getQueue?.('workflow_ingest')
          if (queue?.getJob) {
            ...
            if (job) {
              ...
              return {
                id: runId,
                traceId: (data as { trigger?: { traceId?: string } }).trigger?.traceId ?? null,
                workflowName: (data as { trigger?: { workflowName?: string } }).trigger?.workflowName ?? null,
  ...
  if (enabled('cancel')) {
    r.post('/cancel', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
  ...
  if (enabled('humanInput')) {
    r.post('/human-input', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
  ...
  if (enabled('signal')) {
    r.post('/signal', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
  ...
  if (enabled('query')) {
    r.post('/query', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
  ...
  if (enabled('ingestEvent')) {
    r.post('/ingest-event', wrap(async ({ engine, tenantId }, req) => {
      const handlers = createWorkflowHandlers(engine)
  ...
  if (enabled('listWorkflows')) {
    r.get('/', wrap(async ({ engine, tenantId }) => {
      const handlers = createWorkflowHandlers(engine)

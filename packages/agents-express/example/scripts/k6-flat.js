// k6 flat-rate load test for the Express + agents-express example app.
//
...
export function async() {
  ...
  lat.add(Date.now() - t)
  var success = check(r, { '200': function (x) { return x.status === 200 } })
  if (success) ok.add(1)
  err.add(!success)
}
...
export function handleSummary(data) {
  ...
  return {
    stdout:
      '\n=== ' + MODE.toUpperCase() + ' rate=' + RATE + '/s dur=' + DUR + ' ===\n' +
      '  Workflows OK:    ' + c + ' (' + (c / dur).toFixed(0) + ' wf/s over ' + dur.toFixed(1) + 's)\n' +
      '  Error rate:      ' + (e * 100).toFixed(2) + '%\n' +
      '  Latency p50:     ' + Math.round(m.med) + 'ms\n' +
      '  Latency p95:     ' + Math.round(m['p(95)']) + 'ms\n' +
      '  Latency p99:     ' + Math.round(m['p(99)']) + 'ms\n' +
      '  Latency max:     ' + Math.round(m.max) + 'ms\n',
  }
}

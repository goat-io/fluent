# @goatlab/agents-sandbox

Docker-sandboxed step execution for `@goatlab/agents-core`. Run untrusted code (LLM-generated scripts, Claude Code tasks, third-party integrations) inside isolated containers with network egress locked down by default.
...
```bash
pnpm add @goatlab/agents-sandbox @goatlab/agents-core dockerode
```
...
```ts
import { SandboxStepExecutor } from '@goatlab/agents-sandbox'

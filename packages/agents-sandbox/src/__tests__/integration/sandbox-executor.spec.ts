...
import type { StepPayload } from '@goatlab/agents-core'
...
if (!dockerAvailable) {
  console.warn('⚠️  Docker socket not found — skipping sandbox integration tests')
}

describe('SandboxStepExecutor — Integration', () => {
  it.skipIf(!dockerAvailable)('executes a simple script in a container', async () => {
  ...
  it.skipIf(!dockerAvailable)('runs setup commands before execution', async () => {
    const executor = new SandboxStepExecutor()
  ...
  it.skipIf(!dockerAvailable)('resolves {{workflowRunId}} in setup commands', async () => {
    const executor = new SandboxStepExecutor()
  ...
  it.skipIf(!dockerAvailable)('cleans up container after execution', async () => {
    const executor = new SandboxStepExecutor()
  ...
  it.skipIf(!dockerAvailable)('cleans up container even on setup failure', async () => {
    const executor = new SandboxStepExecutor()
  ...
  it.skipIf(!dockerAvailable)('handles git init and commit inside container', async () => {
    const executor = new SandboxStepExecutor()
  ...
  it.skipIf(!dockerAvailable)('runs Node.js code inside container', async () => {
    const executor = new SandboxStepExecutor()

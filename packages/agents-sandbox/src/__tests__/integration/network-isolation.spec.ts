...
if (!dockerAvailable) {
  console.warn('⚠️  Docker socket not found — skipping network isolation integration tests')
}
...
describe('Network Isolation — Integration', () => {
  ...
  it.skipIf(!dockerAvailable)('networkMode: none blocks all network access', async () => {
  ...
  it.skipIf(!dockerAvailable)('networkMode: bridge allows network access', async () => {
    const payload = makePayload({
      image: 'alpine:latest',
      networkMode: 'bridge',
      execute: {
  ...
  it.skipIf(!dockerAvailable)('allowedDomains restricts traffic to specified domains', async () => {
    const payload = makePayload({
      image: 'alpine:latest',
      networkMode: 'bridge',
      allowedDomains: ['registry.npmjs.org'],
      setup: [
        'apk add --no-cache curl iptables 2>/dev/null || true',
      ],
      execute: {
        type: 'script',
        commands: [
  ...
  it.skipIf(!dockerAvailable)('default networkMode is none (complete isolation)', async () => {
    const payload = makePayload({
      image: 'alpine:latest',
      ...
      execute: {
        type: 'script',
        commands: [
          'ping -c 1 -W 2 8.8.8.8 2>&1 || echo "DEFAULT_BLOCKED"',
        ],
      },
    })

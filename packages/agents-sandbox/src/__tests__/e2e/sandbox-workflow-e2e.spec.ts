...
import type { Database, StepPayload, StepResult } from '@goatlab/agents-core'
...
if (!dockerAvailable) console.warn('Docker not found — skipping sandbox workflow E2E tests')
...
async function waitForWorkflow(
  engine: WorkflowEngine, runId: string, tenantId: string, targets: string[], timeoutMs = 120_000,
): Promise<string> {
  ...
  throw new Error(`Workflow ${runId} stuck at ${final.status} after ${timeoutMs}ms. Steps: ${final.steps.map(s => `${s.stepName}=${s.status}`).join(', ')}`)
}
...
describe('Sandbox Workflow E2E — Real Docker Containers', () => {
  ...
  beforeAll(async () => {
    ...
    db = new Kysely<Database>({
      dialect: new PostgresDialect({
        pool: new pg.Pool({
          host: data.postgres.host, port: data.postgres.port,
          database: data.postgres.database, user: data.postgres.username,
          password: data.postgres.password, max: 10,
        }),
      }),
    })
  ...
  it.skipIf(!dockerAvailable)('runs a 3-step SDLC pipeline in real Docker containers', async () => {
    ...
    const workflow = WorkflowBuilder.create('docker_sdlc')
      .step('analyze', {
        ...
        executorConfig: {
          ...
          execute: { type: 'script' as const, commands: [
    ...
    const engine = await setupEngine([workflow], new Map([['sandbox', sandboxExecutor]]))
    const { runId } = await engine.start({
      workflowName: 'docker_sdlc', tenantId: TENANT,
      input: { task: 'Build auth system' },
    })

    const finalStatus = await waitForWorkflow(engine, runId, TENANT, ['COMPLETED', 'FAILED'])
  ...
  it.skipIf(!dockerAvailable)('mixes sandbox and function executors', async () => {
    const sandboxExecutor = new SandboxStepExecutor()
    const functionExecutor = new FunctionStepExecutor()
    functionExecutor.register('plan', async (): Promise<StepResult> => ({
      output: { tasks: [{ name: 'setup-db' }], planApproved: true },
    }))
    functionExecutor.register('validate', async (): Promise<StepResult> => ({
    ...
    const workflow = WorkflowBuilder.create('mixed_workflow')
      ...
      .step('implement', {
        dependsOn: ['plan'], executorType: 'sandbox',
        executorConfig: {
          image: 'node:20-alpine',
          execute: { type: 'script' as const, commands: [
      ...
      .step('validate', { dependsOn: ['implement'], executorType: 'function', executorConfig: { handler: 'validate' } })
      .build()

    const engine = await setupEngine(
      [workflow],
      new Map<string, any>([['function', functionExecutor], ['sandbox', sandboxExecutor]]),
    )
    const { runId } = await engine.start({ workflowName: 'mixed_workflow', tenantId: TENANT, input: {} })
    const finalStatus = await waitForWorkflow(engine, runId, TENANT, ['COMPLETED', 'FAILED'])

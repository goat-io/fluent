...
it('UpdateById - Should Update a single element', async () => {
  ...
  const data = await GoatModel.remote().updateById(goats[3]._id, {
    age: 99,
    name: 'MyUpdatedGoat'
  })
...
it('FindById - Should get the element by its Id', async () => {
  const insertedGoat = await GoatModel.remote().insert({
    age: 15,
    name: 'MyInsertedData'
  })

  const goat = await GoatModel.remote().findById(insertedGoat._id)
...
it('Select - should select Nested Columns', async () => {
  const goats = await GoatModel.remote()
    .select(GoatModel._keys.name, GoatModel._keys.breed.family)
    .get()
...
it('Skip/Take - should restrict the results', async () => {
  await GoatModel.remote().clear()

  await GoatModel.remote().insert({
    age: 15,
    name: 'Andres'
  })

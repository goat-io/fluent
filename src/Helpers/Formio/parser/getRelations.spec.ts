...
import { AdvancedForm } from './tests/Forms/AdvancedRelationsTest'
import { BasicForms } from './tests/Forms/BasicRelationsTest'
import { MultiplePivots } from './tests/Forms/MultiplePivots'
...
test('Should parse NM relations - NM inverse', () => {
  expect(advancedRelations).toHaveProperty('productsnm')
  expect(advancedRelations.productsnm[1].name).toBe('productsnm_ordersnm_usersnm')
...
test('Should parse multiple NM relations', () => {
  ...
  expect(MultiplePivotsRelations.jobpositions[1].name).toBe('jobpositions_classification_company')
  expect(MultiplePivotsRelations.jobpositions[1].type).toBe('belongsToMany')
  expect(MultiplePivotsRelations.jobpositions[1].pivotTable).toBe('classification')

  expect(MultiplePivotsRelations.jobpositions[2].name).toBe('jobpositions_classification_industries')
  expect(MultiplePivotsRelations.jobpositions[2].type).toBe('belongsToMany')
  expect(MultiplePivotsRelations.jobpositions[2].pivotTable).toBe('classification')
})

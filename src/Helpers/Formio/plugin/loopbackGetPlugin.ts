...
export const loopbackGetPlugin = ({appUrl,localForms, loopbackUrl, formio,form}:LoopbackPluginInterface) => {
    return {
        priority: 1,
        ...
        staticRequest: async (request, options) => {
          ...
          const currentValue = formio.submission.data[lbQueryUrl.formField];
          ...
          if (lbQueryUrl.searchString && lbQueryUrl.where) {
            ...
            if (
              formio.submission.data.country &&
              formio.submission.data.country !== ""
            ) {
            }
            lbQueryUrl.where = lbQueryUrl.where.replace(

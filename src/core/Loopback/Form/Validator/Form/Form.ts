...
const componentKeys = (components: any[]) => {
  ...
  util.eachComponent(
    components,
    (component: any) => {
      if (component.key && !_.isUndefined(component.key) && !_.isNull(component.key)) {
    ...
    schema: new Schema({
      ...
      components: {
        ...
        validate: [
          {
            message: keyError,
            validator: (components: any[]) => componentKeys(components).every((key: string) => key.match(validKeyRegex))
          },
          {
            message: shortcutError,
            validator: (components: any[]) =>
              componentShortcuts(components).every((shortcut: string) => shortcut.match(validShortcutRegex))
          },
          {
            message: msg,
            validator: (components: any[]) => {
          ...
          },
          {
            ...
            validator: (components: any[]) => {
              ...
              const diff = shortcuts.filter((value: any, index: number, collection: any) =>
              ...
              shortcutMsg = shortcutMsg + diff.value().join(', ')
              return false
            }
          }
        ]
